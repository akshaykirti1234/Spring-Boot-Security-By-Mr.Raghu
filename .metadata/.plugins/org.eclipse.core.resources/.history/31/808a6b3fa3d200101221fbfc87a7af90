...
public class SecurityConfig {
	...
	// ========= Authentication =========
	@Bean
	public UserDetailsService configAutentication(DataSource dataSource) {

		UserDetails user1 = User.withUsername("sam")
				.password("$2a$10$e.K89JD8U96rGlP5Kro6Pe/pfyawPAEqkh3sn9CDWlSSxEIZladou").authorities("ADMIN").build();
		UserDetails user2 = User.withUsername("ram")
				.password("$2a$10$qKZ8raOkNyQdVE1kr6xpRuAaczeAuR6DjUHa/7.kfeXyifHVUMlSm").authorities("CUSTOMER")
				.build();
	...
	public SecurityFilterChain configAuthorization(HttpSecurity http) {
		http.authorizeHttpRequests(request -> request.requestMatchers("/", "/home").permitAll()
				.requestMatchers("/admin").hasAuthority("ADMIN").requestMatchers("/customer").hasAuthority("CUSTOMER")
